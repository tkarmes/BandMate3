...
methods: {
  hasValidValue(value, fieldName) {
    console.log(`Checking ${fieldName}:`, value); // Debug log
    if (Array.isArray(value)) {
      // Ensure array is non-null, non-empty, and doesn't contain only empty/whitespace elements
      return value != null && value.length > 0 && value.every(item => item && item.trim() !== '');
    }
    if (fieldName === 'capacity') {
      // Treat 0, "0", empty string, or non-positive numbers as invalid
  ...
  async saveProfile() {
    ...
    try {
      ...
      // Convert space-separated genrePreferences to array, filter empty
      ...
      // Convert space-separated amenities to array, filter empty
